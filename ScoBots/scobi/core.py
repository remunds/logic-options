...
class Environment(Env):
    def __init__(self, env_name, seed=None, focus_dir="focusfiles", focus_file=None, reward_mode=0, hide_properties=False,
                 ...
                 **kwargs):
        self.logger = Logger(silent=silent)
        print("kwargs in scobi core: ", kwargs)
        self.oc_env = em.make(env_name, self.logger, **kwargs)
        print("scobi_core env render: ", self.oc_env.render_mode)
        # self.oc_env = em.make(env_name, self.logger)
    ...
    def _draw_relation_overlay(self, obs_image, feature_vector, freeze_mask, action=None):
        ...
        top_features_names = ["" for _ in range(top_features_k)]
        if np.ptp(self.feature_attribution):
            feature_attribution = (255 * (self.feature_attribution - np.min(self.feature_attribution)) / np.ptp(
                self.feature_attribution)).astype(int)
            top_features_idxs = np.argsort(feature_attribution)[-top_features_k:][::-1]
            for feature in features:
                i += 1
                idxs = np.where(fv_backmap == i - 1)[0]
